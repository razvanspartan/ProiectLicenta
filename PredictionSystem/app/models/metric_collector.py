...
class MetricCollector(MetricCollectorInterface):
    ...
    def collect_metrics(self, instance_name: str, metrics: dict) -> None:
        now = time.time()
        self.instances[instance_name] = {"last_seen": now, "metrics": metrics}
    ...
    def aggregate_metrics(self):
        ...
        if not self.instances:
            return {"cpu_avg": 0, "memory_sum": 0, "requests_per_second": 0}
        cpu_avg = sum(
            metric["metrics"]["cpu"] for metric in self.instances.values()
        ) / len(self.instances)
        memory_sum = sum(
            metric["metrics"]["memory"] for metric in self.instances.values()
        )
        requests_per_second = sum(
            metric["metrics"]["requests_per_second"]
            for metric in self.instances.values()
        ) / len(self.instances)
        data = {
            "cpu_avg": cpu_avg,
