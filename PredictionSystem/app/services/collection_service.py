...
def aggregate_metrics_for_smoothing():
    for (
        ...
        metric_collector,
    ) in metric_collector_factory.metric_collectors.items():
        lightgbm_predictor = lightgbm_factory.get_lightgbm_predictor(
            metric_collector_name
        )
        ...
        print("Predicted value:", prediction_lgbm)
        ...
        try:
            ...
            print(
                f"Error sending scaling decision for {metric_collector_name}: {e}"
            )
        print(
            f"Predicted CPU usage for {metric_collector_name} in 3(5s) steps: {prediction_lgbm}"
        )
        ...
        for container in containers:
            try:
                ...
                if service_name == "loadbalancer":
                    continue
                metric_collector = metric_collector_factory.get_metric_collector(
                    service_name
                )
                response = requests.get(
                    f"http://0.0.0.0:7000/api/v1/loadbalancer/metrics/{service_name}"
                )
                metrics = extract_metrics_from_container(container)
                metrics["requests_per_second"] = response.json().get(
